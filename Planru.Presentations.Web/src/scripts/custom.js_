...
jQuery(document).ready(function () {
    ...
    // Toggle Left Menu
    jQuery('.left-panel .nav-parent > a').live('click', function () {

        var parent = jQuery(this).parent();
        var sub = parent.find('> ul');

        // Dropdown works only when left-panel is not collapsed
        if (!jQuery('body').hasClass('left-panel-collapsed')) {
            if (sub.is(':visible')) {
    ...
    function closeVisibleSubMenu() {
        jQuery('.left-panel .nav-parent').each(function () {
            var t = jQuery(this);
    ...
    // Form Toggles
    jQuery('.toggle').toggle({ on: true });

    jQuery('.toggle-chat1').toggle({ on: false });
    ...
    jQuery('.menu-toggle').click(function () {
        ...
        if (bodypos != 'relative') {
            ...
            if (!body.hasClass('left-panel-collapsed')) {
                ...
                body.removeClass('left-panel-collapsed chat-view');
    ...
    jQuery('#chatview').click(function () {
        ...
        if (bodypos != 'relative') {
            ...
            if (!body.hasClass('chat-view')) {
                body.addClass('left-panel-collapsed chat-view');
                jQuery('.nav-bracket ul').attr('style', '');
                ...
                if (!jQuery('.menu-toggle').hasClass('menu-collapsed')) {
                    jQuery('body').removeClass('left-panel-collapsed');
                    jQuery('.nav-bracket li.active ul').css({ display: 'block' });
                } else {
    ...
    jQuery(window).resize(function () {
        ...
        if (jQuery('body').css('position') == 'relative') {
            ...
            jQuery('body').removeClass('left-panel-collapsed chat-view');
    ...
    function reposition_searchform() {
        if (jQuery('.searchform').css('position') == 'relative') {
            jQuery('.searchform').insertBefore('.left-panelinner .userlogged');
        } else {
    ...
    function reposition_topnav() {
        if (jQuery('.nav-horizontal').length > 0) {
            ...
            if (jQuery('.nav-horizontal').css('position') == 'relative') {
                ...
                if (jQuery('.left-panel .nav-bracket').length == 2) {
                    jQuery('.nav-horizontal').insertAfter('.nav-bracket:eq(1)');
                } else {
                    // only add to bottom if .nav-horizontal is not yet in the left panel
                    if (jQuery('.left-panel .nav-horizontal').length == 0)
                        jQuery('.nav-horizontal').appendTo('.left-panelinner');
                }
    ...
    // Sticky Left Panel
    if (jQuery.cookie('sticky-left-panel')) {
        jQuery('body').addClass('stickyheader');
        jQuery('.left-panel').addClass('sticky-left-panel');
    }

    // Left Panel Collapsed
    if (jQuery.cookie('left-panel-collapsed')) {
        jQuery('body').addClass('left-panel-collapsed');
        jQuery('.menu-toggle').addClass('menu-collapsed');
    }
